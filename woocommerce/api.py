...
class Api(object):
    ...
    async def _request_async(self, endpoint, method='GET', params=None, data=None):
        self.log.debug('requesting %s' % endpoint)
        return await asyncio.get_event_loop().run_in_executor(None, self._request, endpoint, method, params, data)
